define([
    ...
    'app/gallery-page/models/gallery-page-model'
], function ($, angular, Module) {

    Module.controller('postController', ['$scope', 'Language', 'GalleryPage',  function($scope, Language, GalleryPage){
        ...
        $scope.GalleryPage = new GalleryPage();
        ...
        Language.get().then(
            ...
            function(response){
                console.log(response, 'postCtrl - lang not loaded');
            }
        );
        ...
        if ($scope.mod == 'edit' && $scope.post_id != 0){
            $scope.GalleryPage.get($scope.post_id).then(
                function(response){
                    $scope.post = response;
                ...
                function(response){
                    console.log(response, 'GalleryPageCtrl - GalleryPage not loaded');
                }
            );
        }else{
            $scope.GalleryPage.create().then(
                function(response){
                    $scope.post = response;
                    loadingObjects.post = true;
                    checkIfReadyForInit();
                ...
                function(response){
                    console.log(response, 'ArticleCtrl - GalleryPage not loaded');
                }
            );
        }
        ...
        // ------------------------------------
        $scope.parents = [];
        $scope.GalleryPage.getAllParents().then(function(data){
            $scope.parents = data;
        ...
        var setActiveContent = function(){
            if($scope.activeLanguage){
                $scope.GalleryPage.getContent($scope.activeLanguage.code).then(function(){
                    //$scope.GalleryPage.save();
                    setLanguagesStatus();
                });
            }
        };
        ...
        var init = function(){
            ...
            if($scope.GalleryPage.model.parent_id){
                setParent($scope.GalleryPage.model.parent_id);
            }
        };
        ...
        $scope.$watch('GalleryPage.form.parent_id', function(newVal, oldVal){

            if(newVal != oldVal && newVal > 0){
                setParent(newVal);
            }else{
                if($scope.GalleryPage && $scope.GalleryPage.form){
                    $scope.GalleryPage.form.parent = null;
                    $scope.GalleryPage.setParentActiveContent();
                    $scope.GalleryPage.createParentUrl();
                } 
        ...
        var setParent = function(parent_id){
            $scope.GalleryPage.form.parent = _.findWhere($scope.parents, {id: parent_id});

            if(!$scope.GalleryPage.form.parent){
                return;
            }
            $scope.GalleryPage.setParentActiveContent();
            $scope.GalleryPage.createParentUrl();
        ...
        var setLanguagesStatus = function(){
            ...
            var contents = $scope.GalleryPage.form.post_contents;
            // parent contents
            var parentContents = false;

            // set parent contents if parent is set
            if(typeof $scope.GalleryPage.form.parent != 'undefined' && $scope.GalleryPage.form.parent){
                parentContents = $scope.GalleryPage.form.parent.post_contents;
            }
            ...
            if(!data.autosave){
                if($scope.mod == 'new'){
                    if(window.history.replaceState) {
                        window.history.replaceState({}, "Edit Gallery Page", "edit/" + data.model.model.id);
                    }
