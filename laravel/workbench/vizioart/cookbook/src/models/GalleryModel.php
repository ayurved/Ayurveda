...
class GalleryModel extends GalleryDB {
	...
	public function insertItem($params){
		...
		try{
			...
			$item->load('file');
			if($item->type == 'image' && !empty($item->file_id)){
				$image_path = FileHandler::uploadsPath($item->file->url);
				ImageHandler::makeImagePackage($image_path, 'gallery');
			}
	...
	public static function deleteItems($ids){
		...
		try{
			...
			$file_model->delete_safe($file_ids);
